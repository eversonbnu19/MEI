...
assert.match(index,/Gestão de Contratos v34/,'index deve identificar visualmente a versao v34');
assert.match(index,/boot\.js\?v=34/,'index deve carregar boot v34');
assert.match(index,/entry-guard\.js\?v=34/,'index deve carregar guarda de entrada v34');
assert.match(boot,/const APP_VERSION='v34'/,'boot deve declarar v34');
...
assert.match(boot,/app\.js\?v=34/,'boot deve carregar app v34');
assert.match(boot,/patch-preclose-review\.js\?v=34/,'boot deve carregar pre-conferencia v34');
assert.match(boot,/patch-notifications\.js\?v=34/,'boot deve carregar notificações v34');
assert.match(boot,/patch-auditor-erp\.js\?v=34/,'boot deve carregar pendências de ERP v34');
assert.match(boot,/patch-logout-return\.js\?v=34/,'boot deve carregar hotfix de logout v34');
...
assert.doesNotMatch(auditorErp,/__GESTAO_NOTIFY__/,'confirmação no ERP deve usar apenas o disparo global');
...
assert.match(sw,/gestao-contratos-v34/,'service worker deve usar cache v34');
...
console.log('OK - v34 impede notificações duplicadas.');
