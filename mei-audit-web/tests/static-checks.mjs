...
assert.match(index,/Gestão de Contratos v32/,'index deve identificar visualmente a versao v32');
assert.match(index,/boot\.js\?v=32/,'index deve carregar boot v32');
assert.match(index,/entry-guard\.js\?v=32/,'index deve carregar guarda de entrada v32');
assert.match(boot,/const APP_VERSION='v32'/,'boot deve declarar v32');
...
assert.match(boot,/app\.js\?v=32/,'boot deve carregar app v32');
assert.match(boot,/patch-preclose-review\.js\?v=32/,'boot deve carregar pre-conferencia v32');
assert.match(boot,/patch-notifications\.js\?v=32/,'boot deve carregar notificações v32');
assert.match(boot,/patch-auditor-erp\.js\?v=32/,'boot deve carregar pendências de ERP v32');
assert.match(boot,/patch-logout-return\.js\?v=32/,'boot deve carregar hotfix de logout v32');
...
assert.match(sw,/gestao-contratos-v32/,'service worker deve usar cache v32');
...
console.log('OK - v32 com lançamento de NFSe no ERP validado.');
