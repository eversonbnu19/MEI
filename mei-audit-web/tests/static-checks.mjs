...
assert.match(index,/Gestão de Contratos v30/,'index deve identificar visualmente a versao v30');
assert.match(index,/boot\.js\?v=30/,'index deve carregar boot v30');
assert.match(boot,/const APP_VERSION='v30'/,'boot deve declarar v30');
...
assert.match(boot,/app\.js\?v=30/,'boot deve carregar app v30');
assert.match(boot,/patch-preclose-review\.js\?v=30/,'boot deve carregar pre-conferencia v30');
assert.match(boot,/patch-notifications\.js\?v=30/,'boot deve carregar notificações v30');
assert.match(boot,/patch-logout-return\.js\?v=30/,'boot deve carregar hotfix de logout v30');
...
assert.match(notifications,/invoice_received/,'confirmação da NF deve gerar notificação');
assert.match(notifications,/data-notification-retry/,'falhas devem permitir reenvio');
assert.match(sw,/gestao-contratos-v30/,'service worker deve usar cache v30');
...
console.log('OK - v30 com notificações transacionais validada.');
