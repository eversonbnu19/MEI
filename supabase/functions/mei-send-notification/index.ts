...
const label:Record<string,string>={company:'Empresa',mei:'MEI',auditor:'Auditoria'};
const title:Record<string,Record<string,string>>={
  closure_created:{company:'Aguardando NFSe do MEI',mei:'Emitir e enviar NFSe'},
  invoice_sent:{company:'NFSe enviada para conferência',mei:'NFSe enviada à Empresa'},
...
Deno.serve(async request=>{
  ...
  if(body.action==='list'){
    const {data:related}=await admin.from('mei_closures').select('id,company_id,mei_id').or(`mei_id.eq.${user.id}`);
    const companyIds=(related||[]).map(c=>c.company_id);
    const {data:companyUsers}=companyIds.length?await admin.from('mei_company_users').select('company_id').eq('user_id',user.id).in('company_id',companyIds):{data:[]};
    const {data:audits}=companyIds.length?await admin.from('mei_company_auditors').select('company_id').eq('user_id',user.id).in('company_id',companyIds):{data:[]};
    const allowed=new Set([...(related||[]).map(c=>c.id)]);
    if((companyUsers||[]).length||(audits||[]).length){const ids=(companyUsers||[]).concat(audits||[]).map(x=>x.company_id);const {data:closures}=await admin.from('mei_closures').select('id').in('company_id',ids);(closures||[]).forEach(c=>allowed.add(c.id));}
  ...
  if(body.action==='retry'){
    ...
    event=previous.event; closureId=previous.closure_id;
  }
  if(!['closure_created','invoice_sent','invoice_received'].includes(event||'')||!closureId) return json({error:'Evento ou fechamento inválido'},400);
  ...
  const authorized=(event==='closure_created'||event==='invoice_received')?Boolean(isCompany):event==='invoice_sent'&&closure.mei_id===user.id;
  ...
  const recipients=event==='closure_created'?[{role:'company',users:(companyMembers||[]).map(x=>x.user_id)},{role:'mei',users:[closure.mei_id]}]:event==='invoice_sent'?[{role:'company',users:(companyMembers||[]).map(x=>x.user_id)},{role:'mei',users:[closure.mei_id]}]:[{role:'auditor',users:(auditors||[]).map(x=>x.user_id)}];
  const origin=event==='invoice_sent'?'mei':'company';
  ...
  const results=[];
  for(const group of recipients) for(const id of group.users){
    const person=byId.get(id); if(!person?.email) continue;
    const recipientRole=group.role; const subject=`${label[recipientRole]} // ${label[origin]} // Fechamento ${formatDate(closure.period_start)} a ${formatDate(closure.period_end)} // ${title[event!][recipientRole]}`;
    ...
    const html=`<p>Olá, ${escapeHtml(person.name||label[recipientRole])}.</p><p>${escapeHtml(title[event!][recipientRole])}.</p><p><b>Contrato:</b> ${escapeHtml(contract.code)} — ${escapeHtml(contract.service)}<br><b>Período:</b> ${formatDate(closure.period_start)} a ${formatDate(closure.period_end)}<br><b>Valor do fechamento:</b> R$ ${Number(closure.total_value||0).toLocaleString('pt-BR',{minimumFractionDigits:2})}${invoice?`<br><b>NFSe:</b> ${escapeHtml(invoice.invoice_number)}`:''}</p><p>Esta mensagem foi gerada automaticamente pelo sistema Gestão de Contratos.</p>`;
    try{
